...
- Cómo funciona: **toda la lectura ocurre en las cuatro primeras líneas** —días, detalle, sueldos y `obtenerDatosCalculo` en lote— y después no se toca más la base: es el caso donde más se nota la regla del snapshot (6.4). El cálculo lo hace `simulacionMultiple` de `logica/sueldos`. Las recetas del detalle **sin sueldo asignado entran con 0 para el empleado**: asignar una receta a la simulación y no asignarle sueldo significa justamente que el dueño se lleva todo, no es un hueco. Usa el `diasPorSemana` **compartido**, que es distinto del que tiene cada `EmpleadoRecetaSueldo` por separado.
