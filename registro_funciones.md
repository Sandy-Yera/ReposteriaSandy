...
- Cómo funcionan: `traerALaVista` es un `BringIntoViewRequester` puesto sobre el campo, y `disposicionDelTexto` guarda el `TextLayoutResult` que llega por `onTextLayout` — hace falta porque la posición del cursor dentro del texto no dice nada de dónde quedó **en la pantalla**. Un efecto colgado de la selección pide traer el **rectángulo del cursor**, no el campo entero: un paso largo puede ser más alto que lo que queda de pantalla con el teclado abierto, y traerlo completo dejaría a la vista su primera línea, justo la que no importa. Va en un efecto y **no dentro de `onValueChange`** porque ahí la disposición todavía es la del texto anterior: el renglón nuevo aún no se midió, y se pediría mostrar dónde estaba el cursor antes de escribir. Y **solo con el foco puesto**, o cada paso que aparece al desplazar la lista pediría su turno para verse y la lista saltaría sola al abrir la receta. Esto es distinto de lo que hace `imePadding` en la raíz (12.1.1): aquello deja el hueco sobre el teclado, esto mueve la lista.
